...
summarize_glmm <- function(data, 
  ...
  # Build formula
  formula <- as.formula(
    paste0(response, " ~ ", predictor, " + (1|", random, ")")
  )
  ...
  em <- emmeans(model, as.formula(paste0("pairwise ~ ", predictor)), adjust = adjust)
  contrasts_tbl <- tidy(em$contrasts, conf.int = TRUE) |>
    dplyr::mutate(component = "Contrasts") |>
    dplyr::select(component, term = contrast, estimate, std.error, conf.low, conf.high, p.value = adj.p.value)
  
  # Combine all results
  results <- dplyr::bind_rows(coef_tbl, anova_tbl, contrasts_tbl)
  
  # Create gt table
  results |>
    gt(groupname_col = "component") |>
    fmt_number(columns = c(estimate, std.error, conf.low, conf.high), decimals = 4) |>
    ...
    tab_header(title = paste("GLMM Results:", response, "~", predictor))
}
```
