...
labels_bulk <- function(x){str_replace_all(x, c(
  "depth_cm" = "Depth (cm)",
...
labels_AODC <- function(x){str_replace_all(x, c(
...
facet_labels_bulk <- as_labeller(c(
  "depth_cm" = "Depth (cm)",
  "depth_from_org" = "Depth Below Oa (cm)",
  "carbon_perc" = "SOC mg gSoil^-1^",
  "SOC_mgC_gSoil" = "SOC mg gSoil^-1^",
...
facet_labels_bulk_MAOM_sig <- as_labeller(c(
  "depth_cm" = "Depth~(cm)",
  "depth_from_org" = "Depth~Below~Oa~(cm)",
  "carbon_perc" = "SOC~mg~gSoil^-1",
  "SOC_mgC_gSoil" = "SOC~mg~gSoil^-1",
...
partial_tab_func <- function(table, MAOM_type) {
  partial_table <- table |> 
    mutate(MAOM = MAOM_type,
           across(dplyr::where(is.numeric) & !all_of(ends_with("pvalue")),
...
anova_table_fun <- function(model){
  model |> 
    ...
    dplyr::select(-Df) |> 
    dplyr::mutate(across(where(is.numeric), ~case_when(TRUE ~ round(., 3))),
                  signif = case_when(`Pr(>Chisq)` < 0.05 ~ "*",
                                     .default = ""),
...
boxplot_func <- function(data, y){
  ...
  boxplotgraph <- ggplot(data = data,
                         aes(x = factor(HPU, levels = c("DUP", "WUP", "FWL", "ALV")),
                             y = .data[[y]])) +
