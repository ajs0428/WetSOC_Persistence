...
What are the overall SOC stocks including both mineral and organic soils for landscape classes and are they different?
...
readr::write_csv(HLEF_SOC_stock, file = "WetSOC_Persistence/data/dataframes/HLEF_totSOCstock.csv")
...
HFF_SOC_ratio_lm <- lm(log(HFF_SOC_ratio+1) ~ HPU, data = HLEF_stock_cmb)

emmeans(HFF_SOC_ratio_lm, specs = pairwise ~ HPU)
...
lm_FeDC <- glmer((Fe_mg_gSoil_DC) ~ HPU + (1|sample_ID), 
                 data = HLEF_HFF_14C_DC_AO, family = Gamma(link = "log"), 
...
emFeDC$contrasts
print("***************FeAO***************")
lm_FeAO <- glmer((Fe_mg_gSoil_AO) ~ HPU + (1|sample_ID), data = HLEF_HFF_14C_DC_AO, family = Gamma(link = "log"))
...
print("***************AlDC***************")
lm_AlDC <- glmer((Al_mg_gSoil_DC) ~ HPU + (1|sample_ID), data = HLEF_HFF_14C_DC_AO, family = Gamma(link = "log"))
check_assump_func(lm_AlDC)
Anova(lm_AlDC)
emAlDC <- emmeans(lm_AlDC, specs = pairwise ~ HPU, adjust = "tukey")
emAlDC$contrasts
print("***************AlAO***************")
lm_AlAO <- glmer((Al_mg_gSoil_AO) ~ HPU + (1|sample_ID), data = HLEF_HFF_14C_DC_AO, family = Gamma(link = "log"))
...
maoc_depthHPU_lm <- glmer((HFF_mgC_gSoil) ~ HPU*depth_cm +(1|sample_ID), data = HLEF_HFF_trans, family = Gamma(link = "log"))
...
```{r}
maoc_soc <- glmer((HFF_mgC_gSoil) ~ SOC_mgC_gSoil +(1|sample_ID), data = HLEF_HFF_trans, family = Gamma(link = "log"))
summary(maoc_soc)
...
```{r}
HLEF_cor <- round(cor(HLEF_HFF_trans |> dplyr::select(HFF_mgC_gSoil, depth_cm, carbon_perc,
...
```{r}
lmC_MAOM <- glmer((HFF_mgC_gSoil) ~ carbon_perc + (1|sample_ID), data = HLEF_HFF_trans, family = Gamma(link = "log"))
summary(lmC_MAOM)
Anova(lmC_MAOM, type = 3)

# lmTOC_MAOM <- glmer((HFF_mgC_gSoil) ~ TOC + (1|sample_ID), data = HLEF_HFF_trans, family = Gamma(link = "log"))
# summary(lmTOC_MAOM)
# Anova(lmTOC_MAOM, type = 3)

# lmN_MAOM <- glmer((HFF_mgC_gSoil) ~ bulkCN + (1|sample_ID), data = HLEF_HFF_trans, family = Gamma(link = "log"))
...
# lmSiltClay_MAOM <- glmer((HFF_mgC_gSoil) ~ siltclay + (1|sample_ID), data = HLEF_HFF_trans, family = Gamma(link = "log"))
# Anova(lmSiltClay_MAOM, type = 3)

lmpH_MAOM <- glmer((HFF_mgC_gSoil) ~ ph + (1|sample_ID), data = HLEF_HFF_trans, 
                   family = Gamma(link = "log"))
summary(lmpH_MAOM)
...
```{r}
HLEF_HFF_ppcor <- HLEF_HFF_14C_DC_AO |> 
...
pcor(HLEF_HFF_ppcor)$estimate |> 
  ggcorrplot::ggcorrplot(lab = TRUE, lab_size = 2, p.mat = pcor(HLEF_HFF_ppcor)$p.value, )
```
...
lmpH_MAOM_14C <- lmer((delta_14C_adj) ~ ph + (1|sample_ID), data = HLEF_HFF_14C_DithCit_transdf)
...
r.sq(y = HLEF_HFF_14C_DithCit_transdf$HFF_mgC_gSoil,
...
```{r}
HLEF_HFF_14C_DC_AO_ratio <- HLEF_HFF_14C_DC_AO |> mutate(SOC_MAOM_content_ratio = HFF_mgC_gSoil/SOC_mgC_gSoil) 
...
maoc_14C_drv_lm <- glmer((delta_14C_adj) ~ HPU+clay*depth_cm*Al_mg_gSoil+carbon_perc + (1|sample_ID), 
                        data = HLEF_HFF_14C_DithCit_transdf,
                        family = gaussian(link = "identity"))
maoc_14C_drv_lm2 <- glmer((delta_14C_adj) ~  HPU*depth_cm*Al_mg_gSoil+clay+carbon_perc + (1|sample_ID), 
                        data = HLEF_HFF_14C_DithCit_transdf,
                        family = gaussian(link = "identity"))
...
r.sq(y = HLEF_HFF_14C_DithCit_transdf$delta_14C_adj,
