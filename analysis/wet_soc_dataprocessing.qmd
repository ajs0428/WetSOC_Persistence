...
## Import lab excel datasheets from local private drive on computer

- HLEF2022LAB.xlsx is a excel workbook with sheets for
  - LOCATION: lat, lon, HPU, landforms
  - HLEF_FullData: soil depth, BD, rocks, carbon, cn, pH, texture, heavy fractions
  

```{r}

HLEF_HPU <- openxlsx::read.xlsx("/Users/Anthony/Documents/Lab Work/HLEF2022/HLEF2022LAB.xlsx", sheet = "LOCATION") |> 
...
# Just soil carbon stocks
HLEF_SOC_All <- openxlsx::read.xlsx("/Users/Anthony/Documents/Lab Work/HLEF2022/HLEF2022LAB.xlsx", sheet = "HLEF_FullData") |> 
  dplyr::select(c(1:14)) |>
  dplyr::mutate(top = case_when(is.na(depth_cm - lag(depth_cm)) ~ 0,
                ...
                thick = case_when(is.na(depth_cm - lag(depth_cm)) ~ bottom,
                                  (bottom-top) < 0 ~ bottom,
                                  ### uncomment this code to calculate metrics for 100cm only
...
TOC data are in two .csv files: Anthony_Batch1_010224.csv & Anthony_Batch2_010324.csv
  - A name file helps connect lab IDs: HLEF_TOC_NumName.csv

```{r}
toc_batch1 <- read.csv("/Users/Anthony/Documents/Lab Work/SCGSR/data/TOC/Anthony_Batch1_010224.csv")
toc_batch2 <- read.csv("/Users/Anthony/Documents/Lab Work/SCGSR/data/TOC/Anthony_Batch2_010324.csv")
...
From HLEF2022LAB.xlsx - HLEF_FullData
- filter for HFF data
- add columns for HFF mgC/gSoil and silt+clay

```{r}
HLEF_Full <- openxlsx::read.xlsx("/Users/Anthony/Documents/Lab Work/HLEF2022/HLEF2022LAB.xlsx", sheet = "HLEF_FullData") |>
...
```{r}
HLEF_loc <- openxlsx::read.xlsx("/Users/Anthony/Documents/Lab Work/HLEF2022/HLEF2022LAB.xlsx", sheet = "LOCATION", detectDates = TRUE) |> 
...
HLEF_HFF <- HLEF_Full |> 
    left_join(HLEF_loc |> dplyr::select(sample_name, HPU),
    ...
    mutate(SOC_mgC_gSoil = (carbon_perc)*10,
           HFF_mgC_gSoil = (HFF_percC)*(HFF/df_start_wt)*10,
           HFF_percC_of_soilC = ((HFF_percC)*HFF)/((carbon_perc)*df_start_wt),
    ...
    group_by(sample_ID) |> 
    mutate(depth_from_org = case_when(row_number() == 1 ~ (bottom - top)/2,
                                      .default = (bottom - min(top))/2 ) ) |> 
    ungroup() |> 
    dplyr::select(sample_ID, HLEF_ID, HPU, lat, lon, depth_cm, depth_from_org, 
                  # top, bottom, thick, 
                  BD_g_cm3, rock_perc, 
...
### Add ICP-MS and 14C data 

- LLNL_HLEF_Extracts_ICP_MS.xlsx is an excel workbook with sheets for ICP-MS data in ppm 
  - metal extracts are calculated using $\frac{(concentration_{ppm}*(extract volume_{ml}/1000))}{sample weight_{g}}$

```{r}
target_maom14C <- c("HFF_mgC_gSoil", "delta_14C", "delta_14C_adj")

ICP_DC <- openxlsx::read.xlsx("/Users/Anthony/Documents/Lab Work/SCGSR/data/ICP_MS/LLNL_HLEF_Extracts_ICP_MS.xlsx", sheet = 3, detectDates = TRUE) |> 
...
ICP_AO <- openxlsx::read.xlsx("/Users/Anthony/Documents/Lab Work/SCGSR/data/ICP_MS/LLNL_HLEF_Extracts_ICP_MS.xlsx", sheet = 3, detectDates = TRUE) |> 
...
CAMS_14C <- openxlsx::read.xlsx("/Users/Anthony/Documents/Lab Work/SCGSR/data/14C/14C_Dataframe.xlsx", sheet = 1) |> 
...
Add a depth from bottom of O horizon. Basically the top horizon becomes 0 
```{r}
HLEF_HFF_14C_DC_AO_scale <-  HLEF_HFF_14C_DC_AO |> 
  dplyr::mutate(across(where(is.numeric) & !starts_with("SOC_") & !starts_with("HFF_") & !c(delta_14C, `14C.age`, delta_14C_adj, lat, lon), \(x) as.vector(scale(x))))
HLEF_HFF_14C_DC_AO_scale
readr::write_csv(HLEF_HFF_14C_DC_AO_scale, here("data/dataframes/HLEF_HFF_14C_DC_AO_scale.csv"))
